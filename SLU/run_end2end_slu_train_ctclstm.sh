...
SAVE_PATH=TALN2022_${FEATURES_HEAD}_${FEATURES_TYPE}${FEATURES_SPEC}-${FEATURES_LANG}_Ziggurat121-Dec-${DECODER}-${SUBTASK}-H${encoder_size}-Drop-Enc${DROP_RATIO}-Dec${TRANS_DROP_RATIO}_BATCH${BATCH}_LR${LR}_WU${wu_updates}_forAVG5_${CRITERION}_NewInit_SCRATCH_TEST/
if [[ $# -ge 2 ]]; then
	SAVE_PATH=${2}_LR${LR}/
fi
...
PYTHONPATH=${HOME}/work/tools/fairseq/ ${FAIRSEQ_PATH}/fairseq-train ${DATA_PATH} --load-fairseq-encoder TALN2022_FlowBERT-MEDIA-7kl-split_Dec-basic-concept-media_BATCH5_LR0.0005_NewInit_WU0epochs_3LSTM-256_Dropout0.5_StartAnneal1_TEST/checkpoint.best5_avg.pt \
