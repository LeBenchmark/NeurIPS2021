...
def main(featsFolder, jsonPath, modelPath, maxDur, normalised, device):
    """
...
python wav2vec2.py -f "mls_french_base_cut30" -d 29.98 -n False  -j "/home/getalp/alisamis/Datasets/AlloSat/data.json" -m "/home/getalp/nguyen35/flowbert_ssl_resources/wav2vec2.0_models/mls_french_base/checkpoint_best.pt"
    ...
    for i, ID in enumerate(samples.keys()):
        ...
        dim = makeFeatsCsv(waveFullPath, featsFullPath, model, maxDur, normalised, device=device)
        if dim == 0: continue
...
def makeFeatsCsv(wavePath, outPath, model, maxDur, normalised, device="cpu"):
    winlen=0.025; winstep=0.01
    audio_file = AudioSegment.from_wav(wavePath)
    feats = getFeatsFromAudio(wavePath, model, maxDur, normalised, device=device)
...
def getFeatsFromAudio(wavePath, model, maxDur, normalised, device="cpu", convOnly=False): # ADD "DICE" LATER
    # maxDur = 30
    ...
    for audio_file in audio_files:
        ...
        if convOnly: c = model.feature_extractor(sigTensor).transpose(1, 2)
...
if __name__== "__main__":
    ...
    parser.add_argument('--device', '-de', help="which device to use", default="cpu")
    ...
    if Flag:
        ...
        main(args.featsFolder, args.json, args.modelPath, float(args.maxDur), args.normalised, args.device)
