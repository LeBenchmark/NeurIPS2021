...
def reshapeMatrix(mat, newLen, axis=0):
    ...
    newOut = np.zeros((newLen, mat.shape[1]), dtype=float)
    dim = mat.shape[1] if axis == 0 else mat.shape[0]
    for i in range(dim):
        newOut[:, i] = np.interp(np.linspace(0, oldLen, newLen), np.arange(0, len(mat[:,i]), 1), mat[:,i])
